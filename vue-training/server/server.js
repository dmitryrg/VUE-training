...
http.createServer(function (request, response) {
    ...
    if (request.url.substr(0, PREFIX_API_PATH.length) === PREFIX_API_PATH) { //  API-шная часть
                                ...
                                .on('end', () => {
                                    ...
                                    for ( key in requestBodyObj) {
                                        if (!proccessedRecord.hasOwnProperty(key)) {
                                            response.statusCode = 400;
                                            response.end(`Record in db have not field "${key}", API query "${request.method}"`);
                                            return;
                                        }
    ...
    } else { // обычный сервер на выдачу файлов
        ...
        fs.readFile(filePath, (error, contentFile) => {
            if (error) {
                const errSend = (error.code === 'ENOENT' ? {code:404, body:"File is not found"} : {code:500, body:"Server error"});
                response.statusCode = errSend.code;
                response.end (errSend.body);
            }
            else {
...
function findMaxId (contentDb) {
    ...
    contentDb.forEach((record, index) => {
        const recordId = Number(record.id);
        if (maxId === null || maxId < recordId)
            maxId = recordId;
    })
